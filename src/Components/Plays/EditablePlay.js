...
import { updateServerPlay } from "../../api/plays";
...
class EditablePlay extends Component {
  ...
  async createAct(playId, act) {
    ...
    if (response.status >= 400) {
      ...
      let newActs = _.orderBy(
        [...this.state.play.acts, response.data],
        "number"
      );
      this.setState({
  ...
  async createCharacter(playId, character) {
    ...
    if (response.status >= 400) {
      ...
      let newCharacters = _.sortBy(
        [...this.state.play.characters, response.data],
        "name"
      );
      this.setState({
  ...
  async createEntranceExit(actId, sceneId, frenchSceneId, entranceExit) {
    ...
    if (response.status >= 400) {
      ...
      let workingFrenchScene = _.find(workingScene.french_scenes, {
        id: frenchSceneId,
      });
      let newEntranceExits = _.orderBy(
        [...workingFrenchScene.entrance_exits, response.data],
        "line"
      );
      let newFrenchScene = {
        ...workingFrenchScene,
      ...
      workingScene = this.interpolateNewFrenchScene(
        newFrenchScene,
        workingScene
      );
  ...
  async createFrenchScene(actId, sceneId, frenchScene) {
    ...
    if (response.status >= 400) {
      ...
      let newFrenchScenes = _.orderBy(
        [...workingScene.french_scenes, newFrenchScene],
        "number"
      );
      workingScene = {
        ...workingScene,
  ...
  async createOnStage(actId, sceneId, frenchSceneId, onStage) {
    ...
    if (response.status >= 400) {
      ...
      let newOnStages = _.orderBy(
        [...workingFrenchScene.on_stages, response.data],
        "number"
      );
      let newCharacters = [
        workingFrenchScene.characters,
  ...
  async createScene(actId, scene) {
    ...
    if (response.status >= 400) {
      ...
      let workingPlay = this.state.play;
      let workingAct = _.find(workingPlay.acts, { id: actId });
      let newScenes = _.orderBy(
        [...workingAct.scenes, response.data],
        "number"
      );
  ...
  async deleteCharacter(characterId) {
    ...
    if (response.status >= 400) {
      ...
      this.setState({
        play: {
          ...this.state.play,
          characters: this.state.play.characters.filter(
  ...
  async deleteEntranceExit(actId, sceneId, frenchSceneId, entranceExitId) {
    ...
    if (response.status >= 400) {
      ...
      let newFrenchScene = {
        ...workingFrenchScene,
        entrance_exits: workingFrenchScene.entrance_exits.filter(
          (entrance_exit) => entrance_exit.id !== entranceExitId
        ),
      };
  ...
  async deleteFrenchScene(actId, sceneId, frenchSceneId) {
    ...
    if (response.status >= 400) {
      ...
      workingScene = {
        ...workingScene,
        french_scenes: workingScene.french_scenes.filter(
          (french_scene) => french_scene.id !== frenchSceneId
        ),
      };
      workingAct = this.interpolateNewScene(workingScene, workingAct);
      workingPlay = this.interpolateNewAct(workingAct, workingPlay);
  ...
  async deleteOnStage(actId, sceneId, frenchSceneId, onStageId) {
    ...
    if (response.status >= 400) {
      ...
      let newFrenchScene = {
        ...workingFrenchScene,
        on_stages: workingFrenchScene.on_stages.filter(
          (on_stage) => on_stage.id !== onStageId
        ),
      };
  ...
  async updateEntranceExit(actId, sceneId, frenchSceneId, updatedEntranceExit) {
    ...
    if (response.status >= 400) {
      ...
      let workingEntranceExit = _.find(workingFrenchScene.entrance_exits, {
        id: updatedEntranceExit.id,
      });
      let newEntranceExit = { ...workingEntranceExit, ...updatedEntranceExit };

      workingFrenchScene = this.interpolateNewEntranceExit(
        newEntranceExit,
        workingFrenchScene
      );
      workingScene = this.interpolateNewFrenchScene(
  ...
  async updateOnStage(actId, sceneId, frenchSceneId, updatedOnStage) {
    ...
    if (response.status >= 400) {
      ...
      let workingOnStage = _.find(workingFrenchScene.on_stages, {
        id: updatedOnStage.id,
      });
      let newOnStage = { ...workingOnStage, ...updatedOnStage };

      workingFrenchScene = this.interpolateNewOnStage(
        newOnStage,
        workingFrenchScene
      );
      workingScene = this.interpolateNewFrenchScene(
  ...
  interpolateNewEntranceExit = (newEntranceExit, workingFrenchScene) => {
    let workingEntranceExits = workingFrenchScene.entrance_exits.map(
      (entranceExit) => {
  ...
  onEntranceExitCreateFormSubmit = (
    actId,
    sceneId,
    frenchSceneId,
    entranceExit
  ...
  render() {
    ...
    if (!this.state.play.canonical && this.state.production === null) {
      return <div>This play is canonical!</div>;
    }
    return (
      <PlayShow
