...
class EntranceExitsList extends Component {
  ...
  toggleForm = () => {
    this.setState({
      newEntranceExitFormOpen: !this.state.newEntranceExitFormOpen,
    });
  };
  ...
  render() {
    ...
    let entranceExits = (
      <tr>
        <td colSpan="6">'No entrances or exits listed'</td>
      </tr>
    );
    ...
    return (
      <div className="col">
        ...
        {this.state.newEntranceExitFormOpen ? (
          ...
          <Button onClick={this.toggleForm}>Add New Entrance Exit</Button>
        )}
      </div>
    );
  }
