...
class DoublingChart extends Component {
  ...
  getAllActors() {
    let castings = _.filter(this.props.production.jobs, function (job) {
      return (
        job.character &&
  ...
  generateColumns() {
    ...
    if (this.props.level == "act") {
      ...
      filterEmptyContent(getScenesFromPlay(this.props.production.play)).map(
        (scene) => headings.push(scene.pretty_name)
      );
    } else if (this.props.level == "french_scene") {
      filterEmptyContent(
        getFrenchScenesFromPlay(this.props.production.play)
  ...
  generateUncastRow() {
    let blocks = this.getOnStages();
    let jobsThatAreNotCast = _.filter(
      this.props.production.jobs,
      function (job) {
        return (
          job.character &&
          !job.character.name.match(/Could Not Find Character/) &&
