...
class ScriptContainer extends Component {
  ...
  orderText() {
    ...
    let bucket = this.props.text.lines
      .concat(this.props.text.stage_directions)
      .concat(this.props.text.sound_cues);
  ...
  render() {
    if (this.props.text.lines && this.props.text.lines.length > 0) {
      ...
      return (
        <div>
          {lineItems}
          <Button onClick={() => this.props.unloadText()}>
            Hide Text which is test!
          </Button>
        </div>
      );
    }
