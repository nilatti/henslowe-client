...
class PlayScripts extends Component {
    ...
    return (
      <Card key={act.number}>
        ...
        <Accordion.Collapse eventKey={act.number}>
          <Card.Body>
            <div>
              <Button onClick={(event) => this.loadActText(act.id, event)}>
                Load full act text
              </Button>
            </div>
            <div>
              <ListGroup>{scenes}</ListGroup>
            </div>
          </Card.Body>
        </Accordion.Collapse>
      </Card>
  ...
  buildFrenchSceneLi(actNumber, frenchScene, sceneNumber, text) {
    let frenchSceneNumber =
      actNumber + "." + sceneNumber + "." + frenchScene.number;
    return (
      <ListGroup.Item key={frenchSceneNumber}>
        <Button onClick={() => this.loadFrenchSceneText(frenchScene.id)}>
          Load French Scene {frenchSceneNumber} fulltext
        </Button>
      </ListGroup.Item>
    );
  }
  ...
  buildSceneLi(act, scene) {
    let frenchScenes = scene.french_scenes.map((frenchScene) =>
      this.buildFrenchSceneLi(act.number, frenchScene, scene.number)
    );
    let actScene = act.number + "." + scene.number;
    return (
      <ListGroup.Item key={actScene}>
        <Button onClick={() => this.loadSceneText(scene.id)}>
          Load Scene {act.number}.{scene.number} fulltext
        </Button>
        <ListGroup>{frenchScenes}</ListGroup>
      </ListGroup.Item>
    );
  }
  ...
  render() {
    ...
    if (!this.state.play.acts) {
      ...
      if (
        ...
        this.state.play.production &&
        this.state.play.production.lines_per_minute
      ) {
        minutesRunTime = calculateRunTime(
          this.state.text.lines,
          this.state.play.production.lines_per_minute
        ).toFixed(2);
    ...
    return (
      <Col md={12}>
        ...
        <div>
          ...
          this.state.play.production.lines_per_minute ? (
            <strong>
              Run time at {this.state.play.production.lines_per_minute} lines
              per minute: {minutesRunTime} minutes ({hoursRunTime} hours)
            </strong>
          ) : (
            <span></span>
          )}
        ...
        <Col>
          <Row>
            <Col md={3}>
              <div id="play_script">
                <Button onClick={() => this.loadPlayText(play.id)}>
                  Load Full Text
                </Button>
              </div>
            </Col>
